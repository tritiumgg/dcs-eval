...
- **The wait suite fails instead of hanging on a pid-for-stamp `decide`**: its unbounded-wait test now runs on a thread with a 30 s bound, so `cargo test -p dcs-eval wait` reddens `wait/pid-for-stamp` (T54) where it used to hang.
