...
`Answer`'s five arms, `ping` and `probe` on the same window, and the stand-in's
`script`, which stages the `refused` `gui` probe the carrier check refuses.
