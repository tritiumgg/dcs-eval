...
**Finished on 2026-09-22, and retired here.** Every task below is done and every milestone closed;
`docs/PLAN.md` is the plan the build works to now. Nothing here is edited again — it is what was
intended and what was built, and `docs/mutations.md` still files every control under the row it
came from, which is why this document stays where `tools/sweep-cover.sh` can read it.
...
- **A stage is done** when every task in it is done and the stage's named command re-runs them
  ...
  for Rust stages, and for the live stage the decision record that holds each measured row.
...
5. **Every task is marked `developer-only` or `DCS + human` and sequenced on it.** The DCS tasks are
   ...
   with two developer-only rows, T62 and T63, filed there because they were T50's switch, since
   removed (ADR 0031), and T52's verbs.
...
- **Consumer walkers, record grammars, drivers, cursors.** `bridge.md` §7.0 moved `census` and
  ...
  the chunk it evaluates and decides what to walk. This build keeps no catalogue of reads beyond the
  twelve game-state reads (T35, ADR 0031).

**Spikes** (open questions with exit conditions, folded into the live tasks rather than left as
rows), both closed by T50's run on 2026-09-21 (ADR 0031): editor-vs-menu detection, answered by
`MapWindow.getVisible()` in `gui`; and whether the seven opt-in reads are safe from a hook, answered
six yes and `getMissionLoaded` no, sent in turn in one session with the one that stopped the run
retested alone in a fresh one (ADR 0028).
...
- **Milestone D — Proven live in DCS.** Someone can do the whole thing for real: install, evaluate
  ...
  installed through play and a DCS update. *Acceptance:* every Stage 9 figure is in a decision
  record (ADR 0031), and the permanent-install acceptance (T51) passes;
...
needs a real frame, and T62 and T63, the switch T50 turned, since removed, and the verbs T52 runs.
...
| T08 | The session directory, the `<os.time>-<os.getpid>` stamp, and the sibling sweep at load | `lua5.1 tools/harness.lua executor/session` shows a sibling directory removed at load and the own stamp kept; mutations: absent `os.getpid` must refuse to run (visible failure, not a weaker fence); a request in a foreign sibling is never listed; a remove that answers nothing on success, under `executor/framer`'s model of DCS's `os`, read as a refusal reddens the sweep | T07 | developer-only |
| T09 | The reply framer: header/blank/body, publish-by-rename with `.tmp` in the destination, `os.remove` before `os.rename`, request over 262,144 bytes refused unread | `lua5.1 tools/harness.lua executor/framer` prints its count; mutations: writing the final `.res` name directly (no rename) reddens the half-written-not-collected pair; a 300 KiB request parsed as code reddens the size refusal; under a model of DCS's `io` and `os`, which answer some successes with nothing, a bare nil read as a refusal reddens the publish that should land, `false` and a message taken for a silence reddens the host's message, a silent rename, write or close trusted without a stat, or a stale final of the same size taken for the new one, reddens the refusal of the file that did not land, and a take that trusts a silent remove reddens the withheld bytes | T08 | developer-only |
...
| T31 | The handshake and heartbeat readers, and the stand-in publishing both — `standin.rs` left both files for "the readers for them" | `cargo test -p dcs-eval readers` prints its count; a handshake missing `stamp`, one whose `protocol` is not `2`, one carrying a byte past ASCII, and a heartbeat whose `armed` is neither `yes` nor `no` are each refused naming the field, and a heartbeat's age is taken from the file's mtime while `since` stays display-only, being local wall clock with no zone; and a published transport the client would write into inside the install the handshake's `install_guard` names, or inside the variant's Saved Games outside `Logs` — `Config`, `Logs\..\Config`, `LogsX`, a junction under `Logs` — is refused naming the header, while the `%TEMP%` transport and the `Logs` fallback are written into; mutations: an absent `armed` defaulted to `no` reddens the tri-state check; admitting the install, admitting Saved Games outside `Logs`, and refusing `Logs` itself each redden their transport check | T13,T29 | developer-only |
| T54 | Client `collect`/`wait`: a reply on the stamp addressed or discarded as `foreign` (T25 built the executor's half of §4.3), and the §4.4 outcome table, over the 25 ms poll `bridge.md` §3.2 keeps as its fallback | `cargo test -p dcs-eval wait` shows `superseded` on a stamp change, `dead` on a gone PID, `pending`+`waking` for a dormant executor under 10 s, `stalled` past it, `pending` at any age while the phase is `load`, and a reply carrying another session's `stamp` discarded rather than returned; mutations: treating a dormant heartbeat's age as staleness reddens the `armed: no` rule; a `decide` that ignores the re-read stamp, or compares pids in its place, reddens `cargo test -p dcs-eval e2e`, where the shipped executor relaunched over one box, under a new pid or the old one back, must be read `superseded` (the round trip's half T17 defers), and comparing pids also reddens `cargo test -p dcs-eval wait` as a failure, not a hang | T31,T14,T17 | developer-only |
| T32 | Client `status()` (files + PID probe, problems found, the running `app_version` against the build the embedded executor was last measured on — a difference, never a refusal, and `unmeasured` until Stage 9 records one) | `cargo test -p dcs-eval status` shows a foreign-stamp and a foreign-transport heartbeat each reported as a problem and zero round trips, and an `lfs_tempdir` under the client's temp directory — DCS's `%TEMP%\DCS`, measured live — reported as within it and no problem, while one elsewhere, or sharing only its bytes, is one (ADR 0029); and a heartbeat another stamp left before the handshake reported as a leftover and no problem, while one written at or after it stays one (ADR 0030); and a transport the client will not write into reported as a problem; mutations: a round trip issued by `status` reddens the "costs the executor nothing" check; dropping the within branch, admitting every temp directory as within, and containment by byte prefix each redden their temp-directory check; recognising no leftover, reading every foreign heartbeat as one, and reading one at the handshake's instant as one each redden their heartbeat check; not reporting the unwritable transport reddens its check | T31 | developer-only |
...
| T35 | Game-state reads: the chunks (one `pcall` each), the constant read-list, never-send-unlisted; the list grew to twelve when T50's run put the six that answered on by default, added the editor's map in `gui` and dropped `getMissionLoaded` (ADR 0031) | `cargo test -p dcs-eval game_reads` shows each read as its own request under one `pcall`, sent to the state it names; mutation: sending a `DCS.*` name not in the constant list reddens the never-send check | T33 | developer-only |
| T36 | Game-state derivation: the axes, `unknown` as a value, no default arm | `cargo test -p dcs-eval game_state` shows `loading` with no round trip, `paused (read)` noting a disagreeing callback phase, `session: client` on a `refused` `gui` probe, `unknown: <error>` on an errored axis alone, `unknown (the session is loading)` for a read a load held back; mutation: any axis filled from another's evidence reddens a no-default-arm check | T35,T31 | developer-only |
...
| id | task | done when | needs | runs on |
|---|---|---|---|---|
| T37 | `rmcp` stdio wiring, the `serve` role, the client built per call | `cargo test -p dcs-mcp serve` shows only protocol frames on stdout, diagnostics on stderr, and a transport root that appears between two calls resolved by the second, and a call over a handshake naming a transport outside `Logs` refused before anything is written; mutations: a diagnostic written to stdout reddens the transport-cleanliness check; a client resolved once at start-up reddens the appears-between-calls check; a client judged without its write directory reddens the refused-before-writing check | T02 | developer-only |
...
| T44 | `install` places the hook: `Scripts\Hooks\DcsEvalExecutor.lua` written as `.tmp` in that directory and renamed in, over four dispositions — absent, a hash this project shipped (an upgrade, replaced in place and never parked, ADR 0033), and a foreign hash (refused and named unless `--replace`, which parks it). Every other file in that directory is invisible to the placement, the prior project's included (ADR 0022) | `cargo test -p dcs-mcp install` shows `install` twice leaving one hook file, a foreign file refused without `--replace` and parked with it, a shipped-hash file replaced with nothing parked, and a directory holding two hooks that are not ours installed through with neither read nor moved; mutations: writing the final name directly rather than by rename reddens the half-written check; a foreign hash replaced without `--replace` reddens the refusal; a release of ours parked reddens the nothing-parked check | T42,T43,T60 | developer-only |
...
| T45 | `uninstall`: remove exactly what `install` put there, hash-gated, restore parked files but never a release of ours (ADR 0033), and write the register row `uninstalled` | `cargo test -p dcs-mcp uninstall` shows the `Export.lua` line removed by exact match with its neighbours byte-identical, a hook whose hash is not in the shipped list left in place and named, a parked file restored to its original path, a copy of ours an older binary parked left in the store with no hook behind, and the register carrying the `uninstalled` row; mutations: removing a neighbouring line reddens the exact-match check; removing a hook of unknown hash reddens the hash gate; a restore that copies without removing the park reddens the restore check; a park of ours restored reddens the left-in-the-store check | T44,T61,T60 | developer-only |
| T46 | `verify` (which `dcs_status` also runs): the hook's hash against the embedded release's, the `dofile` line present exactly once, no other `DcsEval*` hook beside it and no opinion at all about anyone else's (ADR 0022), `executor.txt` present with `protocol: 2` and its `app_version` reported as a difference and never a refusal, the heartbeat, the `autoexec.cfg` key report, and nothing written | `cargo test -p dcs-mcp verify` reports both policy-gate keys from a fixture, names a second copy of our own executor and a duplicated `dofile` line as problems while leaving another project's hook unnamed, reports a differing `app_version` without refusing, verifies straight after a relaunch the last session's heartbeat outlived (ADR 0030), prints the verdict first over a row per part, with every session fact under the full report (ADR 0032), reports a transport in the variant's `Config` as a problem, and leaves a `git status`-clean fixture tree; mutations: any write during `verify` reddens the clean-tree assertion; a differing `app_version` treated as a failure reddens the difference-not-refusal check; recognising no leftover heartbeat reddens the relaunch check; a verdict line that ignores the problems reddens the not-installed summary check; a session not yet loaded worded `ok` reddens the waiting check; a fact left out of the details reddens the every-fact check; a `verify` that judges the transport without the variant reddens the unwritable-transport check | T42,T44,T61 | developer-only |
...
developer-only both: T62 was the switch T50 turned, removed once T50 had measured it (ADR 0031),
and T63 the installer verbs T52 starts from. They are here rather than in Stages 7 and 8 so that
Milestone C stays as it closed.
...
| T52 | The cutover from `dcs-api-bridge`, by hand and unassisted: its hook deleted and its `Export.lua` line removed before this executor installs (ADR 0022). **Done 2026-09-22** at the maintainer's install, `--variant DCS` among three (ADR 0035) | on a real machine: `DcsApiEval.lua` gone from `Scripts\Hooks\` and its `dofile` line gone from `Export.lua`, then `dcs-mcp install` and `dcs-mcp verify` green, each given `--variant` where `Saved Games` holds more than one; no code checks any of it, and the refusal that once did is removed; mutation: pointing `verify`'s one stray prefix at `dcsapi` instead of `dcseval` must redden — a report that stops naming a second copy of our own executor and starts naming the project this one replaces has undone the narrowing in both directions at once | T63, Milestone C | DCS + human |
| T47 | The first live run: round-trip p50/p95 with the event-driven wait, `cpu_ms`/reply, replies/tick at W=8, seven-state generation wall time | **Measured 2026-09-21** (ADR 0031) with the temporary `dcs-mcp live`, since removed from the binary with its control: the four rows against the recorded baselines (30 ms p50 from the Node client and S4's 14 ms in-Lua median; 465 s/generation), the generation a floor projected from the W=8 `return 1` throughput because this project ships no walker | Milestone C | DCS + human |
| T48 | The dormant cost in DCS three ways (hook absent / installed-dormant / armed-idle), measured in process on the executor's own paths (ADR 0025) | **Measured 2026-09-21** (ADR 0031) with the temporary `dcs-mcp live`, since removed from the binary with its probe suite and control: installed-dormant and armed-idle per frame, and a directory listing timed on the same machine as the incumbent's figure; ADR 0031 records no hook-absent figure and no by-hand whole-frame row; the dormant figure, 0.0011 ms, is at or below the 0.098 ms baseline, which is the incumbent measured on one machine, DCS 2.9.28.26385, one session, so a figure that disagrees on other hardware is a new measurement rather than a regression; a higher figure here reopens `bridge.md` §2 and is reported as such | Milestone C | DCS + human |
| T49 | `missionscripting` through `a_do_script` live with a mission loaded. **Measured 2026-09-21**: answered in a mission, p50 13.9 ms; the s17 flag-agreement fixture is dropped, nothing depending on it (ADR 0031) | `dcs-mcp eval missionscripting …` returns through `a_do_script` with a mission loaded; its round trip timed in a mission with the temporary `dcs-mcp live`, since removed from the binary | Milestone C | DCS + human |
| T62 | **Removed (ADR 0031).** The opt-in reads' switch — `game-state --reads`, `dcs_game_state`'s `reads`, the groups `extra` and `suspect` — built and then taken out when T50's run put every read that answered on by default; its four controls went with it. Every read no axis is made of is still printed a line each | — | T35,T36,T38,T40 | developer-only |
| T50 | The seven opt-in reads each sent alone under the supervisor, in turn in one session, any read that stops the run retested alone in a fresh one (ADR 0028); editor-vs-menu detection, `mission_name` at the menu, the callback vocabulary. **Measured 2026-09-21** (ADR 0031): six answered and are sent by default, `getMissionLoaded` crashed DCS twice and left the list, the editor is told from the menu by `MapWindow.getVisible()` in `gui`, `getMissionName()` is empty at the menu and in the editor and `tempMission` in single player | each read's result, or the crash that named it, taken with the temporary `dcs-mcp live` and in ADR 0031, the instrument since removed from the binary with its six controls; `dcs_game_state` answers `at the main menu (read)` or `in the mission editor (read)` from the editor's map, `menu-or-editor` only where `gui` refuses it, and names a mission by its tidied file where the name is `tempMission`, with its theatre and the player's unit; mutations: `getMissionLoaded` back in the list reddens the dropped-read check, the editor's map read in `hook` or unguarded in `gui` reddens the gui-read checks, the map read backwards or a refused read taken for the menu reddens the editor checks, and `tempMission` shown as the name, a doubled slash kept, or the theatre or unit dropped reddens the mission checks | Milestone C | DCS + human |
| T51 | Permanent-installation acceptance: install, fly with the executor dormant, survive a DCS update, `verify` still green. **Passed 2026-09-22** at the maintainer's install; Milestone D closes on it, and ADR 0035 holds what shipped unmeasured | on a real machine: `dcs-mcp install`, given `--variant` where `Saved Games` holds more than one; a play session with the executor dormant and no noticeable frame impact; a DCS update leaving `Saved Games` untouched; `dcs-mcp eval` in every reachable state; `dcs-mcp game-state` reporting the state; `dcs-mcp verify` green afterward — the project's done-condition | T52,T47,T48,T49,T50 | DCS + human |

**Stage command:** none now. The temporary `dcs-mcp live report` printed T47 to T50's rows and left
the binary once ADR 0031 held them; T51 and T52 were by hand.  **Milestone D acceptance:** T51 passes and
the dormant per-frame cost (T48) is at or below baseline. **Met 2026-09-22** (ADR 0035).
