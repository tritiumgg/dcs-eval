...
| T26 | The events log `B|`/`O|` markers and rotation to `events.prev.log` at load, and the mission door's own markers through `log.write` inside `mission` (T21 built the door without them) | `lua5.1 tools/harness.lua executor/events` shows the last `B|` with no `O|` naming the killer and one generation kept, and a door crossing marked in `dcs.log` before and after `a_do_script`; mutation: a synthetic unbalanced file whose killer is misread reddens the reader | T12,T21 | developer-only |
