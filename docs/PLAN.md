...
| T50 | The seven opt-in reads each sent alone under the supervisor, in turn in one session, any read that stops the run retested alone in a fresh one (ADR 0028); editor-vs-menu detection, `mission_name` at the menu, the callback vocabulary. **Measured 2026-09-21** (ADR 0031): six answered and are sent by default, `getMissionLoaded` crashed DCS twice and left the list, the editor is told from the menu by `MapWindow.getVisible()` in `gui`, `getMissionName()` is empty at the menu and in the editor and `tempMission` in single player | `dcs-mcp live report` prints a row per read sent alone, with its result or the crash that named it; `dcs_game_state` answers `at the main menu (read)` or `in the mission editor (read)` from the editor's map, `menu-or-editor` only where `gui` refuses it, and names a mission by its tidied file where the name is `tempMission`, with its theatre and the player's unit; mutations: a second read admitted in one session reddens the one-per-session check, a read written to the ledger only once it is answered reddens the ledger-first check, the read published beside a ping reddens the alone check, a sequence that sends past a read that did not answer reddens the stop check, a read skipped in a scene it was never sent in, or a scene's result hidden in the report, reddens the scene checks; `getMissionLoaded` back in the list reddens the dropped-read check, the editor's map read in `hook` or unguarded in `gui` reddens the gui-read checks, the map read backwards or a refused read taken for the menu reddens the editor checks, and `tempMission` shown as the name, a doubled slash kept, or the theatre or unit dropped reddens the mission checks | Milestone C | DCS + human |
