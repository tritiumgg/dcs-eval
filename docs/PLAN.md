...
| T25 | The stamp fence, the executor's half of §4.3: `for` ≠ stamp → `stale-session`, chunk never runs (the reply whose `stamp` is not the session addressed is discarded by the client, and moved to T31 with the `collect` it needs) | `lua5.1 tools/harness.lua executor/fence` shows a foreign `for` answered `stale-session` without running, at load and on a tick; mutation: running the chunk anyway reddens the kill-reproduction control | T10 | developer-only |
