...
-- globals. The second is an `a_do_script` of the suite's own in that
-- `mission`: it records what it was passed, compiles the far chunk in a
-- modelled `missionscripting`, runs it with the arguments, and answers
-- with the return list shifted as DCS shifts it, `v1 … vN` as
-- `nil, v1 … v(N-1)`. What that proves is the carrier under the reference
-- interpreter against the shift as it was measured; what DCS does with
-- it is a live install's to say, and an `a_do_script` that is not the
-- measured one is refused here and read as such there.
--
...
-- what the near chunk was handed in slot 2 was a string each time. It
-- reads slot 2: the far chunk returned the payload and `0`, and an
-- `a_do_script` that does not shift is refused with the types of both
-- slots named. The backstop: a payload in slot 2 that is not a string, a
-- table among them, is refused under `stage: a_do_script` without a key
-- of it read. The body crosses into `mission` once, as a `%q` literal,
-- and into `missionscripting` as an argument; the far chunk is the same
-- bytes for every request and holds nothing a requester wrote. Line 47
-- is true through both hops. The chunk runs in `missionscripting`'s
-- globals. The ceiling is applied beyond `a_do_script`. With no mission
-- loaded the reply is `no-mission`, read at the moment of use: loaded, a
-- request crosses, and unloaded again, the next is `no-mission` again.
-- An `a_do_script` that raises, a far chunk handed no arguments, and the
-- near chunk's own raise are each answered. The first hop's answers are
-- the carrier's: `refused`, `invalid-state`, the model's empty answer,
-- and a host without `net`. Every reply carries `carrier: a_do_script`
-- and `via: mission`. The export host serves no `missionscripting`.
--
...
-- the backstop, and an `a_do_script` that does not shift is read as an
-- unshaped answer rather than `stage: a_do_script`. Read slot 1 instead
-- of slot 2, or drop the sacrificial `0`, and that first crossing reads a
-- nil payload. Put the body in the far chunk, and it appears twice in
-- what crosses into `mission`. Keep `a_do_script` from a previous
-- crossing, and one replaced between requests is not the one called.
-- Drop the carrier's two headers from a refusal, or read `no-mission` as
-- an error, and the header checks read which.
local t = ...
...
-- source, the arguments, what the far chunk returned, and what it
-- answered. `shift` false answers what the far chunk returned unshifted,
-- a DCS build that corrected it.
local function shifting(far, crossings, shift)
...
local function opened()
  ...
  install(mission, shifting(far, crossings))
...
do
  ...
  -- A table first: converted beyond `a_do_script`, so the backstop
  ...
  t.eq(v.result_type, "table", "table: typed beyond a_do_script")
  ...
  t.eq(v.status, "ok", "string: a chunk through a_do_script is answered")
  t.eq(v.result_type, "string", "string: typed")
  t.eq(v.chunkname, "=dcs-eval", "string: the default chunkname is echoed")
  t.eq(v.carrier, "a_do_script", "string: the carrier is a_do_script")
  ...
  t.eq(#crossings, 1, "string: and a_do_script was called once")

  -- Every type a chunk can return, and what the near chunk was handed for it.
  local cases = {
...
do
  ...
  -- A far chunk that escaped the wrapper: the near chunk is handed a table in
  ...
  t.eq(v.stage, "a_do_script", "number: a number in slot 2 is refused too, not printed")
  ...
  t.eq(v.stage, "a_do_script", "dropped: an empty slot 2, a lone value the shift dropped, is refused")
  t.check(body:find("^slot 1 is nil and slot 2 is nil,"), "dropped: by type: " .. body)

  -- A payload that is a string and not in the shape is the carrier's, not
  -- the state's.
  install(mission, function()
...
do
  ...
  -- A body that is code in the near chunk's own terms stays a string.
  ...
  t.eq(v.stage, "compile", "quote: compiled beyond a_do_script, and nowhere before it")

  t.eq(E.raised, 0, "literal: nothing reached the guard")
end

--------------------------------------------------------------------------------
-- Line numbers and globals beyond a_do_script
--------------------------------------------------------------------------------
...
do
  ...
  local _
  _, v, body = eval(E, frame, "5-c", "", "far_seen = 7")
  t.eq(v.status, "ok", "globals: a chunk that sets a global runs")
  t.eq(rawget(far, "far_seen"), 7, "globals: and it landed in missionscripting")
  t.eq(rawget(mission, "far_seen"), nil, "globals: not in mission")
  _, v, body = eval(E, frame, "5-d", "", "return far_seen")
  t.eq(body, "7", "globals: the next chunk beyond a_do_script reads it")
...
--------------------------------------------------------------------------------
-- The ceiling is applied beyond a_do_script
--------------------------------------------------------------------------------
...
do
  ...
  t.eq(v.status, "no-mission", "shut: no mission loaded is no-mission, a status of its own")
  ...
  t.eq(body, "crossed", "loaded: a mission loaded defines a_do_script")
  install(mission, nil)
  order, v, body = eval(E, frame, "7-c", "", 'return "crossed"')
  fields(order, HEAD, "unloaded")
  t.eq(v.status, "no-mission", "unloaded: and unloading it clears it again")
  t.eq(#crossings, 1, "unloaded: a_do_script was called for the loaded mission alone")

  install(mission, true)
  _, v, body = eval(E, frame, "7-d", "", "return 1")
  t.eq(v.status, "no-mission", "not a function: an a_do_script that is not a function answers no-mission")
...
do
  ...
  -- An a_do_script that runs the far chunk and hands it nothing.
  local far = t.state("missionscripting", {})
...
do
  -- The near chunk's own raise, the executor's failure: a mission with no
  -- `rawget` under it.
  local E, env, _, frame = loaded("hook")
  local hops = {}
...
do
  ...
  t.eq(v.status, "error", "gui: no-mission from another state is not a status there")
  ...
  t.eq(v.status, "unsupported", "no net: a host without net.dostring_in has no a_do_script")
...
do
  ...
  t.eq(v.status, "unsupported", "export: the export host has no a_do_script")
