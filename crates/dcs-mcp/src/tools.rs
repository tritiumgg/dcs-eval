...
//! through one small renderer below, so the change that settles it is a
//! change to one function rather than to six bodies.
...
use dcs_eval::pipeline::{PipeError, Pipeline, Spec};
...
/// Every answer these six give is rendered here: a status word, then a line
/// each.
///
/// Deliberately plain, and deliberately one function. What a reply must say
/// — a refusal that reads as a refusal rather than as an empty result, a
/// `pending` that names its id and its phase — is a separate question from
/// which tools exist, and keeping the rendering in one place is what lets the
/// answer to it change one function.
fn say(status: &str, lines: Vec<String>) -> CallToolResult {
...
fn refuse(status: &str, lines: Vec<String>) -> CallToolResult {
...
fn answered(item: Option<Result<Outcome, PipeError>>) -> CallToolResult {
    match item {
        Some(Ok(Outcome::Reply(envelope))) => say("reply", reply_lines(&envelope)),
        Some(Ok(Outcome::Pending { id, phase, flag })) => {
