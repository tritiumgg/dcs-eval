...
-- the body. `door-shut` is the mission door finding no mission, a status
...
local function answer(req, chunkname, status, detail, body, tail)
  ...
  elseif status == "unsupported" or status == "door-shut" then
...
-- The mission door: the one way into `missionscripting`, which
-- `net.dostring_in` reaches under no name. It takes two hops. The first
-- is `net.dostring_in` into `mission`, carrying `DOOR`; the second is
-- `DOOR` calling `a_do_script`, a global of `mission` that runs a chunk
...
-- once, as a `%q` literal in `DOOR`, and into `missionscripting` as a
...
-- with a second, sacrificial value, `0`, and `DOOR` reads the payload out
-- of slot 2. Where the payload in slot 2 is not a string, `DOOR` refuses
...
--
-- Nothing but a string crosses the door. A table returned through
-- `a_do_script` has corrupted DCS's allocator, the damage surfacing
-- during the call, at mission teardown or on the next load, so a clean
-- crossing clears nothing. The wrapper converts the result inside
-- `missionscripting`, which is the mechanism; the slot-2 refusal is the
-- backstop for a far chunk that escaped it, and not the mechanism.
--
-- `a_do_script` is nil at the main menu and whenever no mission is
-- loaded, and `DOOR` reads it at the moment of use, never from a
-- previous crossing, because this executor outlives any one mission.
-- Finding none is `door-shut`. `DOOR` answers in the wrapper's three
-- fields, adding two statuses of its own: `door-shut`, and `door` for a
-- door that opened and did not hand back a string, or raised. Its own
-- raise is `bridge`, as the wrapper's is.
...
local DOOR_HEAD = "return (function(far, body, chunkname) local opened, answered = pcall(function() "
  .. 'local door = rawget(_G, "a_do_script") '
  .. 'if type(door) ~= "function" then return "door-shut\\n\\nno mission is loaded: a_do_script is " '
  .. '.. type(door) .. " in the mission state, and the door opens only with a mission loaded" end '
  .. "local crossed, lead, payload = pcall(door, far, body, chunkname) "
  .. 'if not crossed then return "door\\n\\na_do_script raised: " '
  .. '.. (type(lead) == "string" and lead or "(error object is a " .. type(lead) .. " value)") end '
  .. 'if type(payload) ~= "string" then return "door\\n\\nslot 1 is " .. type(lead) .. " and slot 2 is " '
  .. '.. type(payload) .. ", where a_do_script\'s shift puts a nil in slot 1 and the string payload in slot 2" end '
  .. "return payload end) "
  .. "if opened then return answered end "
  .. 'return "bridge\\n\\n" .. (type(answered) == "string" and answered or type(answered)) end)('

local function door(body, chunkname)
  return DOOR_HEAD .. string.format("%q", FAR) .. ", " .. string.format("%q", body) .. ", "
    .. string.format("%q", chunkname) .. ")"
end

-- The headers every reply through the door carries, so a reader knows
-- the result crossed two hops and could only have been a string.
local DOOR_TAIL = { { "carrier", "a_do_script" }, { "via", "mission" } }

-- The statuses a wrapper sends back, so that a string in the three-field
-- shape by accident is not read as one, and the door's, which adds two.
...
local DOOR_STATUS = { ["door-shut"] = true, door = true }
for status in pairs(WRAPPER_STATUS) do
  DOOR_STATUS[status] = true
end
...
--
-- The mission door is this carrier too, into `mission` with `DOOR` in
-- place of the wrapper. Every reply it answers carries `DOOR_TAIL`, the
-- door's own statuses are read, and a string in neither shape is `stage:
-- door`, because `DOOR` hands back only its own answers or the payload.
local function eval_dostring(req, chunkname, state, through_door)
  local chunk, statuses, unshaped, tail = wrapper(req.body, chunkname), WRAPPER_STATUS, "dostring_in", nil
  if through_door then
    chunk, statuses, unshaped, tail = door(req.body, chunkname), DOOR_STATUS, "door", DOOR_TAIL
  end
  local net = rawget(_G, "net")
...
-- is reached through the mission state's door, one hop past `mission`.
